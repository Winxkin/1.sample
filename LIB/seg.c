...
void seg_set (GPIO_TypeDef* GPIOx, uint8_t Data)
{
	...
	set_bit (GPIOx, a, buf[0]);
	set_bit (GPIOx, b, buf[1]);
	set_bit (GPIOx, c, buf[2]);
	set_bit (GPIOx, d, buf[3]);
	set_bit (GPIOx, e, buf[4]);
	set_bit (GPIOx, f, buf[5]);
	set_bit (GPIOx, g, buf[6]);
	set_bit (GPIOx, dp, buf[7]);
}
